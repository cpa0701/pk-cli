<template>
  ...
  <Form ref="userFormSearch" :model="baseModel" :label-width="80" inline class="form search-form">
    ...
    <FormItem :label-width="0">
      <Button type="success" icon="md-add" @click="userAdd">新 增</Button>
    </FormItem>
    <FormItem :label-width="0">
      <Button type="text" @click="userRemove">删 除</Button>
    </FormItem>
...
export default {
  ...
  methods: {
    ...
    userAdd() {
      this.$parent.$refs.userAddDrawer.userDrawerShow()
    },
    userHandleReset() {
    ...
    userRemove() {
      this.$parent.removeSelectList('uid')
    }
  }
}
</script>
